...
- [ ] Gravar vídeo (até 15 min, YouTube/Vimeo) demonstrando GA + LLM e resultados
...
- [ ] Rodar `LLM/llm_integration.py --sample-count 8 --openai-model gpt-4.1-mini` com API real
- [ ] Substituir outputs de fallback em `llm_outputs/` pelos gerados com LLM real
- [ ] Registrar no relatório que a integração foi executada com LLM real
...
- [ ] Substituir todos os `print()` por `logging.info()` / `.warning()` / `.error()` em `GA/ga_optimizer.py`, `LLM/llm_integration.py`, `models/export_baselines.py`
- [ ] Configurar nível de log via variável de ambiente `LOG_LEVEL`
- [ ] Garantir que tempo de execução e métricas fiquem registrados em log
- **Referência:** Requisito 2 — Monitoramento e logging adequados para tracking de desempenho

### T06 — Documentação de arquitetura
- [ ] Criar `docs/arquitetura.md` com diagrama/descrição dos componentes (GA, LLM, models, results)
- [ ] Documentar decisões de implementação (ex: por que regressão logística no GA, escolha do DEAP, modo fallback)
- [ ] Descrever fluxo de dados: dataset → baseline → GA → LLM → outputs
